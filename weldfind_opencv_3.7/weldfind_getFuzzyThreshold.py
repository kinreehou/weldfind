...
class getThreshold:
    ...
    def getCannyThreshold(self):
        if self.T>self.range1:
            T1=self.calculateFuzzyThreshold(self.range1,self.T-1)
        else:
            T1=self.T
...
class imgToThresholdValues():
    def __init__(self,imgPath):
        self.imgPath = imgPath
    
    def getThresholdValues(self):
        testObject = getHistogram('p1.png')
        testObject_Hist=testObject.getHistList()
        testObject_getThreshold=getThreshold(testObject_Hist)
        return (testObject_getThreshold.getCannyThreshold())


#testall = imgToThresholdValues('p1.png')
#print(testall.getThresholdValues())
